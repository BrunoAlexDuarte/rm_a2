...
class MapDrive(MarkerVisualization):
	def __init__(self):
		...
		rospy.Subscriber("/map_metadata", MapMetaData, self.sendRvizPoints)
	...
	def sendRvizPoints(self, map):
		resolution = map.resolution * 2;
		origin_x = map.origin.position.x * resolution;
		origin_y = map.origin.position.y * resolution;
		left_low = (origin_x, origin_y);
		left_high = (origin_x, -origin_y);
		right_high = (-origin_x, -origin_y);
		right_low = (-origin_x, origin_y);
		self.rvizPointReceived(left_low);
		self.rvizPointReceived(left_high);
		self.rvizPointReceived(right_high);
		self.rvizPointReceived(right_low);
		self.rvizPointReceived(left_low);
