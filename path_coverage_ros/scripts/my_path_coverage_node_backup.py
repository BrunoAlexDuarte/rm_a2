...
class MapDrive(MarkerVisualization):
	def __init__(self):
		...
		rospy.Subscriber("/map_metadata", MapMetaData, self.mapCorners);
	...
	def mapCorners(self, mapdata):
		if self.global_costmap is None or self.local_costmap is None:
			rospy.logerr("No global or local costmap, doing nothing.")
			return
		origin = mapdata.origin.position
		left_low = (origin.x, origin.y)
		left_high = (origin.x, -origin.y)
		right_high = (-origin.x, -origin.y)
		right_low = (-origin.x, origin.y)
		points = [left_low, left_high, right_high, right_low, left_low]
		...
		if len(points) > 2:
			...
			#	raise ValueError()
			points_x = [p.point.x for p in points]
			points_y = [p.point.y for p in points]
